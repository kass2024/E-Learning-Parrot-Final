...
const MeetingRegistration = () => {
  ...
  const inputClass = (invalid?: boolean) =>
    cn(
      ...
      "transition-all duration-200 focus-visible:ring-2 focus-visible:ring-[var(--institution-primary,#38761D)]/20 focus-visible:border-[var(--institution-primary,#38761D)]",
  ...
  if (isInstitutionPortal && institutionLoading) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-white">
        <Loader2 className="h-9 w-9 animate-spin text-[var(--institution-primary,#38761D)]" />
      </div>
    );
  }
    ...
    >
      <main className={cn(isInstitutionPortal ? "pb-16 px-4 pt-10" : "public-page-offset pb-16 px-4")}>
        <div className="container mx-auto max-w-5xl">
          ...
          >
            <Badge className="mb-4 bg-[var(--institution-primary,#38761D)]/8 text-[var(--institution-primary,#38761D)] border-[var(--institution-primary,#38761D)]/15 hover:bg-[var(--institution-primary,#38761D)]/10">
              <Video className="h-3.5 w-3.5 mr-1.5" />
              Book a session
            </Badge>
            <h1 className="text-3xl md:text-4xl font-bold text-[var(--institution-primary,#38761D)] mb-2">{bookingTitle}</h1>
            ...
            <div className="mt-8 flex items-center justify-center gap-2 sm:gap-4">
              <div className={cn("flex items-center gap-2 px-4 py-2 rounded-full text-xs font-semibold uppercase tracking-wider transition-colors", step === 1 ? "bg-[var(--institution-primary,#38761D)] text-white shadow-md shadow-[var(--institution-primary,#38761D)]/20" : "bg-white text-slate-500 border border-slate-200")}>
              ...
              <div className={cn("flex items-center gap-2 px-4 py-2 rounded-full text-xs font-semibold uppercase tracking-wider transition-colors", step === 2 ? "bg-[var(--institution-primary,#38761D)] text-white shadow-md shadow-[var(--institution-primary,#38761D)]/20" : "bg-white text-slate-500 border border-slate-200")}>
          ...
          {step === 1 ? (
            ...
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
              <Card className="rounded-3xl border border-slate-200/80 bg-white shadow-xl shadow-slate-200/50 overflow-hidden">
                <div className="h-1 bg-gradient-to-r from-[var(--institution-primary,#38761D)] via-[#E01C21] to-[var(--institution-primary,#38761D)]" />

                <CardHeader className="pb-4 pt-8 px-6 md:px-8">
                  <CardTitle className="text-xl font-bold text-[var(--institution-primary,#38761D)]">Your information</CardTitle>
                  {selectedSlot && learnerTimezone && (
                    <div className="mt-4 rounded-2xl border border-[var(--institution-primary,#38761D)]/10 bg-gradient-to-br from-[var(--institution-primary,#38761D)]/5 to-white p-4">
                      <div className="flex gap-3">
                        <div className="shrink-0 rounded-xl border border-[var(--institution-primary,#38761D)]/15 bg-white px-2.5 py-2 text-center min-w-[52px] shadow-sm">
                          <CalendarDays className="h-4 w-4 text-[var(--institution-primary,#38761D)] mx-auto" />
                        </div>
                        <div className="min-w-0 flex-1">
                          <p className="text-sm font-semibold text-[var(--institution-primary,#38761D)] leading-snug">
                            {formatSelectedMeeting(selectedSlot.startsAt, learnerTimezone)}
                            <button
                              type="button"
                              onClick={() => setStep(1)}
                              className="ml-2 text-[#E01C21] hover:underline font-semibold"
                            >
                              Edit
                            </button>
                          </p>
                          <p className="mt-1.5 flex items-center gap-2 text-xs text-slate-600">
                            <Video className="h-3.5 w-3.5 text-[var(--institution-primary,#38761D)]" />
                  ...
                  <form className="space-y-8" onSubmit={handleSubmit}>
                    <section className="space-y-4">
                      <div className="flex items-center gap-2 pb-1 border-b border-slate-100">
                        <User className="h-4 w-4 text-[var(--institution-primary,#38761D)]" />
                        <h3 className="text-sm font-semibold text-[var(--institution-primary,#38761D)] uppercase tracking-wide">Your details</h3>
                      </div>
                    ...
                    <section className="space-y-4">
                      <div className="flex items-center gap-2 pb-1 border-b border-slate-100">
                        <MessageSquare className="h-4 w-4 text-[var(--institution-primary,#38761D)]" />
                        <h3 className="text-sm font-semibold text-[var(--institution-primary,#38761D)] uppercase tracking-wide">
                          ...
                          return (
                            <button
                              ...
                              className={cn(
                                ...
                                active
                                  ? "border-[var(--institution-primary,#38761D)] bg-[var(--institution-primary,#38761D)]/5 text-[var(--institution-primary,#38761D)] shadow-sm"
                              ...
                              <span className="flex items-center gap-2">
                                <span
                                  className={cn(
                                    ...
                                    active ? "border-[var(--institution-primary,#38761D)]" : "border-slate-300"
                                  )}
                                >
                                  {active && <span className="h-2 w-2 rounded-full bg-[var(--institution-primary,#38761D)]" />}
                                </span>
                                {reason.label}
                              </span>
                            </button>
                    ...
                    <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                      <div className="flex items-start gap-3">
                        <Checkbox
                          ...
                          className="mt-0.5 data-[state=checked]:bg-[var(--institution-primary,#38761D)] data-[state=checked]:border-[var(--institution-primary,#38761D)]"
                        />
                    ...
                    <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-between border-t border-slate-100 pt-6">
                      <Button
                        ...
                        variant="outline"
                        onClick={() => setStep(1)}
                        className="rounded-full border-[var(--institution-primary,#38761D)] text-[var(--institution-primary,#38761D)] hover:bg-[var(--institution-primary,#38761D)]/5"
                      >
                      ...
                      <Button
                        type="submit"
                        disabled={submitting}
                        className="rounded-full bg-[var(--institution-primary,#38761D)] hover:bg-[var(--institution-primary-dark,#2A5F17)] text-white font-semibold px-8 h-12 shadow-lg shadow-[var(--institution-primary,#38761D)]/20"
                      >
                        {submitting ? (
