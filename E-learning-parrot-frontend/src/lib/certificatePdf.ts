...
export async function downloadCertificatePdf(input: CertificatePdfInput): Promise<void> {
  ...
  const issuer = input.issuer ?? "parrotglobalstudyacademy";
  ...
  // Header — centered Parrot logo + title
  ...
  const metaLines = [
    ...
    "Platform: parrotglobalstudyacademy Learning",
  ];
  ...
  doc.setFontSize(7);
  doc.text("Authorized by parrotglobalstudyacademy", metaLeft, sigLineY + 4);
  ...
  doc.save(`Parrot-Certificate-${safeName}.pdf`);
}
