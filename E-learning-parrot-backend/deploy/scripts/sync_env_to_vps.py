...
COPY_KEYS = [
    ...
    # MoPay — Xander-only merchant; never reuse F&R credentials/callback/receiver
    ...
    "MOPAY_CALLBACK_SIGNING_KEY", "MOPAY_CALLBACK_URL", "MOPAY_DEFAULT_COUNTRY_CODE",
    "MOPAY_DEFAULT_MNO", "MOPAY_DEFAULT_CURRENCY", "MOPAY_RECEIVER_ACCOUNT_NO",
    "MOPAY_PAYMENT_TITLE", "MOPAY_PAYMENT_DETAILS",
    ...
    "GOOGLE_AI_API_KEY", "GOOGLE_AI_MODEL", "GEMINI_MODEL", "GEMINI_STT_MODEL",
...
FORCE = {
    ...
    # Xander MoPay identity (credentials still come from local .env when set)
...
def main() -> int:
    local = parse_env(LOCAL_ENV.read_text(encoding="utf-8", errors="replace"))
    vps = load_vps()
    ...
    print("STRIPE_SECRET_KEY=", "SET" if merged.get("STRIPE_SECRET_KEY") else "EMPTY")
    print("MOPAY_PROJECT_SLUG=", merged.get("MOPAY_PROJECT_SLUG"))
    print("MOPAY_CALLBACK_URL=", merged.get("MOPAY_CALLBACK_URL"))
    print("MOPAY_AUTH_KEY=", "SET" if merged.get("MOPAY_AUTH_KEY") else "EMPTY")
    print("MOPAY_RECEIVER_ACCOUNT_NO=", "SET" if merged.get("MOPAY_RECEIVER_ACCOUNT_NO") else "EMPTY")
...
sleep 4
docker exec parrot_backend printenv | grep -E '^(DAILY_|APP_URL|FRONTEND_URL|STRIPE_|MOPAY_PROJECT|MOPAY_CALLBACK|MOPAY_AUTH)=' | sed -E 's/(DAILY_API_KEY|STRIPE_SECRET_KEY|MOPAY_AUTH_KEY)=.*/\1=***/'
